...
var DataSetApi = (function () {
    ...
    DataSetApi.prototype.updateDataSet = function (name, dataSetUpdate, extraHttpRequestParams) {
        var path = this.basePath + '/api/DataSets/{name}'
            .replace('{' + 'name' + '}', String(name));
        var queryParameters = new http_1.URLSearchParams();
        var headerParams = this.defaultHeaders;
        if (name === null || name === undefined) {
            throw new Error('Required parameter name was null or undefined when calling updateDataSet.');
        }
        var requestOptions = {
            method: 'PUT',
            headers: headerParams,
